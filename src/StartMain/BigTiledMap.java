...
public class BigTiledMap {
	 ...
	 */
	public BigTiledMap(String ref) throws SlickException {
		this(ref, true);
	}
	 ...
	 *             Indicates a failure to load the tilemap
	 */
	public BigTiledMap(String ref, boolean loadTileSets) throws SlickException {
	 ...
	 *             Indicates a failure to load the tilemap
	 */
	public BigTiledMap(String ref, String tileSetsLocation) throws SlickException {
	 ...
	 *             Indicates a failure to load the tilemap
	 */
	public BigTiledMap(InputStream in) throws SlickException {
		load(in, "");
	}
	 ...
	 *             Indicates a failure to load the tilemap
	 */
	public BigTiledMap(InputStream in, String tileSetsLocation)
			throws SlickException {
	...
	public int getLayerIndex(String name) {
		...
		for (int i = 0; i < layers.size(); i++) {
			BigLayer layer = (BigLayer) layers.get(i);
	...
	public Image getTileImage(int x, int y, int layerIndex) {
		BigLayer layer = (BigLayer) layers.get(layerIndex);

		int tileSetIndex = layer.data[x][y][0];
		if ((tileSetIndex >= 0) && (tileSetIndex < tileSets.size())) {
			BigTileSet tileSet = (BigTileSet) tileSets.get(tileSetIndex);
	...
	public String getTileProperty(int tileID, String propertyName, String def) {
		...
		BigTileSet set = findTileSet(tileID);
	...
	public void render(int x, int y, int sx, int sy, int width, int height,
		...
		BigLayer layer = (BigLayer) layers.get(l);
	...
	protected void renderIsometricMap(int x, int y, int sx, int sy, int width,
			int height, BigLayer layer, boolean lineByLine) {
		...
		while (!allProcessed) {
			...
			for (int burner = 0; burner <= min; currentTileX++, currentTileY--, burner++) {
				for (int layerIdx = 0; layerIdx < drawLayers.size(); layerIdx++) {
					BigLayer currentLayer = (BigLayer) drawLayers.get(layerIdx);
	...
	private void load(InputStream in, String tileSetsLocation)
		...
		try {
			...
			for (int i = 0; i < layerNodes.getLength(); i++) {
				...
				BigLayer layer = new BigLayer(this, current);
				layer.index = i;
	 ...
	 */
	public BigTileSet getTileSet(int index) {
		return (BigTileSet) tileSets.get(index);
	}
	 ...
	 */
	public BigTileSet getTileSetByGID(int gid) {
		for (int i = 0; i < tileSets.size(); i++) {
			BigTileSet set = (BigTileSet) tileSets.get(i);
	 ...
	 *         defined
	 */
	public BigTileSet findTileSet(int gid) {
		for (int i = 0; i < tileSets.size(); i++) {
			BigTileSet set = (BigTileSet) tileSets.get(i);
